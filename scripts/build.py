...
def parse_personal_blogs(readme: str) -> list[dict]:
    """Parse Personal Blogs section from README."""
    section = re.search(
    ...
    # Matches: | [Name](url) | tag1, tag2 |
    # Automatically ignores empty lines, headers (| Name | Tags |), and separators (|---|---|)
    pattern = r"\|\s*\[([^\]]+)\]\(([^)]+)\)\s*\|\s*([^|]*?)\s*\|"
    ...
    return [
        ...
        for name, url, tags in re.findall(pattern, section.group(1))
    ]
